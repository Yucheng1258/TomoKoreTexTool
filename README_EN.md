...
本项目基于原作者 Timimimi 的 [TomoKoreFacepaintTool](https://github.com/Timiimiimii/TomoKoreFacepaintTool)使用AI进行了改进
...
| **双默认地址** | 支持两个默认导出路径，选择默认地址时将文件同时复制到两处 |
| **存储路径持久化** | 修改默认保存路径后重启脚本仍然生效，通过本地配置文件实现。两个默认地址可分别修改 |
...
4. 输入文件编号（0–999）
5. 选择存放位置：
   - **导出到两个默认地址** — 同时复制到默认地址1和默认地址2
   - **自定义路径** — 复制到用户指定目录
   - **不移动** — 保留在 PNG 所在目录
   - **修改默认地址1 / 修改默认地址2** — 分别修改两个默认路径
...
```
将工具生成的文件导出到这个两个文件路径进行替换，进入游戏后即可生效

注：开始前请先对游戏的原文件进行备份防止意外

## 许可证

本项目沿用原作者的许可证。详见[原始仓库](https://github.com/Timiimiimii/TomoKoreFacepaintTool)。
